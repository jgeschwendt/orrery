...
- Every LiveView renders its own full-screen chrome via `OrreryWeb.UI` (`<.rail>`, `<.ph>` phosphor icons, `<.message>`/`<.block>`) — there is no `Layouts.app` wrapper and no core_components.ex.
